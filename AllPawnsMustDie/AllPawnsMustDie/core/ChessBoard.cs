...
namespace AllPawnsMustDie
{
    ...
    public class ChessBoard
    {
        ...
        public String FEN_Chess960()
        {
            ...
            char[] piese = new char[9];
            int index = random.Next(list.Count - 2);
            piese[index] = 'r';
            list[index] = 0;
            ...
            else
            {
                ...
                while (true)
                {
                    index3 = random.Next(list.Count);
                    if (index3 < index2 && index < index3)
                    {
                        piese[index3] = 'k';
                        list[index3] = 0;
                        break;
            ...
            {
                index2 = random.Next(list.Count);
                if (index % 2 != index2 % 2 && list[index2] != 0)
                {
                    piese[index2] = 'b';
                    list[index2] = 0;
                    break;
            ...
            FEN = FEN + "/pppppppp/8/8/8/8/PPPPPPPP/";
            if (ok == 0)
                for (i = 0; i < 8; i++)
                    FEN = FEN + char.ToUpper(piese[i]);
            else
                for (i = 7; i >= 0; i--)
                    FEN = FEN + char.ToUpper(piese[i]);
            FEN = FEN + " w KQkq - 0 1";
